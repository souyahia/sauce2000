...
const sauceResultsIntros: ((n: number) => string)[] = [
  ...
  (n: number) => `I once tried to create a religion based around sauce. Didn't work out well. I am still in contact with an old fanatic of mine though. I gave him a call, he told me the ${n > 1 ? `top ${n}` : ''} most possible sauce${n > 1 ? 's' : ''} for this image ${n > 1 ? 'are' : 'is'} :`,
